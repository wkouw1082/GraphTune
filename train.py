...
from models import cvae, cvae_for_2_tuples, re_encoder
...
def train_cvae(params, args, logger):
    ...
    logger.info("モデル概要")
    logger.info(
        summary(model, input_size=(params.model_params["batch_size"], train_dataset.shape[1], train_dataset.shape[2]),
...
def train_cvae_2_tuples(params, args, logger):
    """CVAEを学習する関数(2-tuples version)
    ...
    # 前処理後のdataをloadし, 加工する
    train_dataset = joblib.load("dataset/train/onehot_2_tuples")
    train_label = joblib.load("dataset/train/label_2_tuples")
    train_conditional = joblib.load("dataset/train/conditional_2_tuples")
    valid_dataset = joblib.load("dataset/valid/onehot_2_tuples")
    valid_label = joblib.load("dataset/valid/label_2_tuples")
    valid_conditional = joblib.load("dataset/valid/conditional_2_tuples")
    time_size, node_size, edge_size, conditional_size = joblib.load("dataset/param")
    ...
    # モデル選択
    model = cvae_for_2_tuples.CVAE(dfs_size=dfs_size, time_size=time_size, condition_size=conditional_size, params=params,
                              device=device)
    if args.checkpoint_file and args.init_epoch:  # モデルをloadする場合
        model.load_state_dict(torch.load(args.checkpoint_file, map_location="cpu"))
        init_epoch = int(args.init_epoch)  # 初期エポック数
    ...
    for epoch in range(init_epoch, params.epochs + 1, 1):
        ...
        for phase in ["train", "valid"]:
            ...
            for i, (indicies, data) in enumerate(dataloader, 0):
                ...
                # 訓練の時だけ、履歴を保持する
                with torch.set_grad_enabled(phase == 'train'):
                    mu, sigma, tu, tv = model(data, word_drop=params.word_drop_rate)
...
def train_cvae_with_pre_trained_re_encoder(params, args, logger):
    ...
    if args.use_model == "VAEwithReEncoder":
        ...
        logger.info(summary(model, input_size=(
            params.model_params["batch_size"], train_dataset.shape[1], train_dataset.shape[2]),
                            col_names=["output_size", "num_params"]))
        logger.info(summary(re_encoder_model,
                            input_size=(params.model_params["batch_size"], train_dataset.shape[1], dfs_size - 1),
                            col_names=["output_size", "num_params"]))
    else:
        ...
        for phase in ["train", "valid"]:
            ...
            for i, (indicies, data) in enumerate(dataloader, 0):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    ## Model loss
                    ## model_lossは, 各lossのミニバッチのsumに, 対応するalphaなどの定数を乗じて, 和をとったものである.
                    model_loss = model.loss(encoder_loss, decoder_total_loss) + params.model_params[
                        "gamma"] * re_encoder_loss.item()
                    model_loss_per_epoch[phase] += model_loss.item()
            ...
            if phase == "train":
                ...
                for (key, val) in decoder_acc_per_epoch_dict[phase].items():
                    ...
                    logger.info(
                        f"    {key}_acc = {val / math.ceil(train_data_num / params.model_params['batch_size'])}")
                writer.add_scalar(f"{phase}_loss/re_encoder_loss", re_encoder_loss_per_epoch[phase] / train_data_num,
                                  epoch)
                ...
                for (key, val) in decoder_acc_per_epoch_dict[phase].items():
                    ...
                    logger.info(
                        f"    {key}_acc = {val / math.ceil(valid_data_num / params.model_params['batch_size'])}")
                writer.add_scalar(f"{phase}_loss/re_encoder_loss", re_encoder_loss_per_epoch[phase] / valid_data_num,
                                  epoch)
...
def train_re_encoder(params, args, logger):
    ...
    model = model.to(device)  # modelをGPUに乗せる
    logger.info("モデル概要")
    logger.info(summary(model, input_size=(params.model_params["batch_size"], train_dataset.shape[1], dfs_size - 1),
                        col_names=["output_size", "num_params"]))
    ...
    for epoch in range(init_epoch, params.epochs + 1, 1):
        ...
        for phase in ["train", "valid"]:
            ...
            for i, (indicies, data) in enumerate(dataloader, 0):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if torch.isnan(model_loss):
                        ...
                        ## nanを検知した時点でのmodelとoptを保存する
                        torch.save(model.state_dict(),
                                   "result/" + params.run_date + "/train/nan_iter_" + str(i) + "_weight_" + str(epoch))
                        torch.save(opt.state_dict(),
                                   "result/" + params.run_date + "/train/nan_opt_iter_" + str(i) + "_weight_" + str(
                                       epoch))
                        ## nanを検知した時点の1 iter前のmodelとoptを保存する
                        torch.save(model.state_dict(),
                                   "result/" + params.run_date + "/train/pred_nan_iter_" + str(i) + "_weight_" + str(
                                       epoch))
                        torch.save(opt.state_dict(), "result/" + params.run_date + "/train/pred_nan_opt_iter_" + str(
                            i) + "_weight_" + str(epoch))
                        ## プログラム終了
                        exit()
                    else:
            ...
            if phase == "train":
                ...
                # for logger
                logger.info(
                    f"    model_loss_per_epoch / train_data_num = {model_loss_per_epoch[phase] / train_data_num}")
                # for csv
                ...
                # for logger
                logger.info(
                    f"    model_loss_per_epoch / valid_data_num = {model_loss_per_epoch[phase] / valid_data_num}")
                # for csv
...
if __name__ == "__main__":
    ...
    train_cvae_2_tuples(params, args, logger)
